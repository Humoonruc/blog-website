...
```{python}
print(time.time()) # 获取当前时间戳：从1970年1月1日到现在的秒数
type(time.time())

print(time.ctime()) # 人能读的字符串
type(time.ctime())

print(time.gmtime()) # 计算机能处理的时间格式，一个对象
type(time.gmtime())
```
...
# strftime() 的第二个参数为 gmtime() 格式（time.struct_time类对象）的时间，返回第一个参数格式的字符串

timeStr = '2019-09-26 10:35:24'
time.strptime(timeStr, '%Y-%m-%d %H:%M:%S')
# strptime()的第一个参数为时间字符串，第二个参数为格式化模板，返回gmtime()格式的时间
```

# 程序计时

```{python}
start = time.perf_counter() # perf_counter()多用于取差值求得时间间隔
time.sleep(1) # sleep()让程序休眠一段时间
