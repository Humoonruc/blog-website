...
创建画布`setup()`、结束绘画`done()`

## 画笔控制函数

`penup()`/`pu()`、`pendown()`/`pd()`、`pensize()`、`pencolor()`、`colormode(0.0~1.0/0~255)`、`color()`、`beginfill()`、`endfill()`、`hideturtle()`

## 运动控制函数

`speed(1~10)`

移动：`goto(绝对坐标)`、`forward()/fd()`、`circle()`

角度：`seth(绝对角度)`、`left(相对角度)`、`right(相对角度)`

## 其中两个比较复杂的函数

### `turtle.setup(width, height, startx, starty)`

以屏幕左上角为原点，单位为像素。后两个参数若不给出，默认将画布居于屏幕正中。

海龟默认出生在画布的正中央，坐标为 (0, 0)
