...
# 投入产出表数据比较特殊，它本质上是四个维度的数据(部门维度有两个)
# 对其进行特殊处理
...
EX = X .* τ̃₁₉₉₃ # X本来只有进口数据，乘以关税才是对外国的支出数据
...
# 总工资收入
Yₙˡ = @pipe (γʲ .* Y) |> mapslices(sum, _; dims=3) # 对产业加总即可
# 关税转移支付，先对i加总，再对产业加总
...
αs = @pipe (final_demand ./ Iₙ) |>
           replace(x -> x < 0 ? 0 : x, _)
αs = αs ./ mapslices(sum, αs; dims=3) # 因为将负数变成了0，各产业α总和未必是1，要重新归一化
